...
func NewNodeTeleport(cfg *pcconfig.Config) (*PocketNodeTeleportProcess, error) {
    var err error
    process := &PocketNodeTeleportProcess{
...
type PocketNodeTeleportProcess struct {
    sync.Mutex
    service.Supervisor
...
func (p *PocketNodeTeleportProcess) findStaticIdentity(id auth.IdentityID) (*auth.Identity, error) {
...
func (p *PocketNodeTeleportProcess) connectToAuthService(role teleport.Role) (*service.Connector, error) {
...
func (p *PocketNodeTeleportProcess) onExit(callback func(interface{})) {
    go func() {
...
func (p *PocketNodeTeleportProcess) initSSH() error {
...
func (p *PocketNodeTeleportProcess) RegisterWithAuthServer(token string, role teleport.Role, eventName string) {
    cfg := p.Config
...
*/
            ...
            // TODO when it's necessary to bring local connectivity on OSX, we'll bring in 1) LocalAuth connectivity or 2) combine PocketCoreTeleportProcess & PocketNodeTeleportProcess together
...
// from the server to get a pair of SSH keys signed by Auth server host
// certificate authority
func (p *PocketNodeTeleportProcess) RequestSignedCertificateWithAuthServer(token string, role teleport.Role, eventName string) {
    cfg := p.Config
    identityID := auth.IdentityID{Role: role, HostUUID: cfg.HostUUID}
