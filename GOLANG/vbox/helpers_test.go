...
func WithDvdInVm(t *testing.T, isoName string, disableBootPrompt bool, testCase func(Machine, Session, Console)) {
    medium := Medium{}
    ...
    if isoName != "" {
        ...
        medium, err = OpenMedium(imageFile, DeviceType_Dvd, AccessMode_ReadOnly, false)
        if err != nil {
            t.Fatal(err)
        }
        defer func() {
    ...
    vmpath, err := ComposeMachineFilename("pwnall_vbox_test", "", os.TempDir())
    if err != nil {
        t.Fatal(err)
    }
    machine, err := CreateMachine(vmpath,"pwnall_vbox_test", "Ubuntu", "")
    if err != nil {
        t.Fatal(err)
    }
    defer machine.Release()

    // NOTE: Using the USB 1.1 controller so tests can run on the open-sourced VirtualBox (no extension packs).
    ...
    defer func() {
        ...
        // TODO(pwnall): Figure out how to get rid of this timeout. The VM should be unlocked, according to the check above, but unregistering the VM fails if we don't wait.
...
func AddDvdToMachine(t *testing.T, machine Machine, medium Medium, session Session) {
