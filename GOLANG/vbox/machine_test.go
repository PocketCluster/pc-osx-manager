...
func TestCreateMachine(t *testing.T) {
    ...
    machine, err := CreateMachine(goldenPath,"pwnall_vbox_test", "Linux", "")
    if err != nil {
        t.Fatal(err)
    }
    ...
    vmpath, err := machine.GetSettingsFilePath()
    if err != nil {
        t.Error(err)
    } else if vmpath != goldenPath {
        t.Error("Wrong settings path: ", vmpath, " expected: ", goldenPath)
    }
...
func TestMachine_SaveSettings(t *testing.T) {
    ...
    machine, err := CreateMachine(vmpath,"pwnall_vbox_test", "Linux", "")
    if err != nil {
        t.Fatal(err)
    }
    defer machine.Release()
