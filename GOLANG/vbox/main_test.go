...
func TestMain(m *testing.M) {
    ...
    // We use Boot2docker
    isoFile := path.Join(testDir, "boot2docker.iso")
    if _, err = os.Stat(isoFile); err != nil {
        fmt.Printf("Lubuntu ISO not found, downloading\n")
        response, err := http.Get("https://github.com/boot2docker/boot2docker/releases/download/v17.06.1-ce-rc2/boot2docker.iso")
        if err != nil {
            fmt.Printf("%v\n", err)
            os.Exit(1)
        }
        ...
        fmt.Printf("Done downloading boot2docker ISO\n")
    }
