...
class acbuf
{
    public:
        ...
        //! Returns capacity
        inline size_t freecapa() const
        {
            return m_nCapacity - w;
        }
    
        inline size_t totalcapa() const
        {
            return m_nCapacity;
        }
         ...
         */
        ssize_t syswrite(int fd, size_t maxlen = SIZE_MAX);
    ...
    protected:
        // read/write positions, size
        size_t r, w, m_nCapacity;
        char *m_buf;
        // don't copy me
        acbuf(const acbuf&);
...
class tSS : public acbuf
{
    public:
        ...
        inline tSS& operator<<(const char* val)
        {
            return add(val);
        }
    
        inline tSS& operator<<(const mstring& val)
        {
            return add(val);
        };
    
        inline tSS& operator<<(const acbuf& val)
        {
            return add(val.rptr(), val.size());
        };
        
        #define __tss_nbrfmt(x, h, y) \
        { \
            add(fmtbuf, sprintf(fmtbuf, m_fmtmode == hex ? h : x, y)); \
            return *this; \
        }
        inline tSS& operator<<(int val)
            __tss_nbrfmt("%d", "%x", val);
    
        inline tSS& operator<<(unsigned int val)
            __tss_nbrfmt("%u", "%x", val);
    
        inline tSS& operator<<(long val)
            __tss_nbrfmt("%ld", "%lx", val);
    
        inline tSS& operator<<(unsigned long val)
            __tss_nbrfmt("%lu", "%lx", val);
    
        inline tSS& operator<<(long long val)
            __tss_nbrfmt("%lld", "%llx", val);
    
        inline tSS& operator<<(unsigned long long val)
            __tss_nbrfmt("%llu", "%llx", val);

#ifdef DEBUG
        inline tSS & operator<<(void* val)
            __tss_nbrfmt("ptr:%llu", "ptr:0x%llx", (long long unsigned) val);
#endif
        ...
        inline tSS& operator<<(fmtflags mode)
        {
            m_fmtmode=mode;
            return *this;
        }
        ...
        inline tSS(size_t sz)
            :m_fmtmode(dec)
        {
            setsize(sz);
        }
    
        inline tSS(const tSS &src)
            :acbuf()
            ,m_fmtmode(src.m_fmtmode)
        {
            add(src.data(), src.size());
        }
    
        tSS& addEscaped(const char *fmt);
    
        inline tSS& operator<<(const char c)
        {
            reserve(size() + 1);
            *(wptr()) = c;
            got(1);
    ...
    protected:
        char fmtbuf[22];
        fmtflags m_fmtmode;
        inline void reserve(size_t minCapa)
        {
            minCapa += (r + 1);
            if (m_nCapacity>=minCapa)
                return;
            char *p = (char*) realloc(m_buf, std::max(m_nCapacity, minCapa * 2));
            if (!p)
                throw std::bad_alloc();
            m_nCapacity = minCapa * 2;
            m_buf = p;
        }
        
        inline tSS& add(const mstring& val)
        {
