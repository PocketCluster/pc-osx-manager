...
class fileitem : public condition
{
    public:
        ...
        enum FiStatus
        {
            ...
            ,FIST_DLERROR
            // assumed to not have any users left
            ,FIST_DLSTOP
        };
        ...
        virtual bool DownloadStartedStoreHeader(const header& h,
        ...
        FiStatus GetStatusUnlocked(off_t &nGoodDataSize)
        {
            nGoodDataSize = m_nSizeChecked;
            return m_status;
        }
...
{
    public:
        inline fileitem_with_storage(cmstring &s)
        {
            m_sPathRel = s;
        };
...
class fileItemMgmt
{
    public:
        ...
        inline fileitem* operator->() const
        {
            return m_ptr.get();
        }
    
        inline tFileItemPtr & get()
        {
            return m_ptr;
        }
