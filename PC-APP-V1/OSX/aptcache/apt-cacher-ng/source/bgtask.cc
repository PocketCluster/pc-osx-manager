...
void tSpecOpDetachable::Run()
{
    ...
    if (m_parms.cmd.find("&sigabort")!=stmiss)
    {
        ...
        {
            tSS buf(255);
            buf << "HTTP/1.1 302 Redirect\r\nLocation: " << m_parms.cmd.substr(0,nQuest)<< "\r\nConnection: close\r\n\r\n";
    ...
    if (m_szDecoFile && (deco.initFromFile((acfg::confdir+SZPATHSEP+m_szDecoFile).c_str()) || (!acfg::suppdir.empty() && deco.initFromFile((acfg::suppdir+SZPATHSEP+m_szDecoFile).c_str()))))
    {
    ...
    else
    {
        ...
        SendFmt << "Maintenance task <b>" << GetTaskName() << "</b>, apt-cacher-ng version: " ACVERSION;
        SendFmtRemote << " (<a href=\"" << m_parms.cmd << "&sigabort=" << rand() << "\">Cancel</a>)";
        SendFmt << "<br>\n";
        ...
        if (!m_delCboxFilter.empty())
            SendChunkRemoteOnly(NAMEWLEN("<br><b>Action(s):</b> "
        ...
        SendFmtRemote << "<br>\n<a href=\"/"<< acfg::reportpage<<"\">Return to main page</a></form>";
        auto& f(GetFooter());
