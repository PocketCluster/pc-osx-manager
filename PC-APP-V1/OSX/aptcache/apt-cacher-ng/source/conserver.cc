...
#define MAX_BACKLOG 200
    ...
    inline bool SpawnThreadsAsNeeded()
    {
        ...
        int nNeeded = static_cast<int>(Qu.size() - g_nStandbyThreads);
    ...
    void CreateUnixSocket() {
        ...
        g_sockunix = socket(2, SOCK_STREAM, 0);
        setsockopt(g_sockunix, SOL_SOCKET, SO_REUSEADDR, &jo, sizeof(jo));
        
        if (g_sockunix<0)
            die();
        
        if (::bind(g_sockunix, (struct sockaddr *)&addr_unx, static_cast<socklen_t>(size)) < 0)
            die();
