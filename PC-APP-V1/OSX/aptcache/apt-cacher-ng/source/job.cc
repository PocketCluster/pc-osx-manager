...
public:
    ...
    ssize_t SendData(int out_fd, int, off_t &nSendPos, size_t nMax2SendNow)
    {
        if (m_status > FIST_COMPLETE || out_fd<0)
            return -1;
        ssize_t r = m_data.syswrite(out_fd, nMax2SendNow);
        if (r > 0)
            nSendPos += r;
        return r;
    }
...
#define GOTOENDE { m_state=STATE_FINISHJOB ; continue; }
                    ...
                    ssize_t n = m_pItem->SendData(confd, m_filefd, m_nSendPos, nMax2SendNow);
                ...
                case(STATE_SEND_CHUNK_DATA):
                {
                    ...
                    if(m_nChunkRemainingBytes==0)
                        GOTOENDE; // done
                    ssize_t n = m_pItem->SendData(confd, m_filefd, m_nSendPos, m_nChunkRemainingBytes);
                    if(n < 0)
                        THROW_ERROR("400 Client error");
                ...
                case(STATE_SEND_BUFFER):
                {
                    ...
                    MYTRY
                    {
                        ...
                        ssize_t r = send(confd, m_sendbuf.rptr(), m_sendbuf.size(), m_backstate == STATE_TODISCON ? 0 : MSG_MORE);
                        if (r < 0)
                        {
