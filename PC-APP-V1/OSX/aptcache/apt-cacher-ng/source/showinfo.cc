...
void tMarkupFileSend::Run()
{
    ...
    if(!m_bFatalError)
    {
        m_bFatalError = !(fr.OpenFile(acfg::confdir+SZPATHSEP+m_sFileName, true) || (!acfg::suppdir.empty() && fr.OpenFile(acfg::suppdir+SZPATHSEP+m_sFileName, true)));
    }
...
#define BADCHARS "<>\"'|\t"
    ...
    for(const auto path : files)
    {
        ...
        {
            sHidParms << "<input type=\"hidden\" name=\"kf" << ++lfd << "\" value=\"" << path <<"\">\n";
        }
        else
        {
