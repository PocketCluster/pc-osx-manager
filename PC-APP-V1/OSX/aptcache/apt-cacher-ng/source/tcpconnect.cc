...
void termsocket(int fd)
{
    ...
    while (true)
    {
        //CHECK : is it safe to static_cast size_t?
        int r = static_cast<int>(recv(fd, buf, 40, MSG_WAITALL));
        if(0 == r)
            break;
        if(r < 0)
        {
...
tTcpHandlePtr tcpconnect::CreateConnected(cmstring &sHostname
                                          ,cmstring &sPort
                                          ,mstring &sErrOut
                                          ,bool *pbSecondHand
                                          ...
                                          ,int timeout)
{
    LOGSTART2s("tcpconnect::CreateConnected", "hostname: " << sHostname << ", port: " << sPort << (bSsl?" with ssl":" , no ssl"));
...
bool tcpconnect::SSLinit(mstring &sErr, cmstring &sHostname, cmstring &sPort)
{
    ...
    hret = static_cast<int>(SSL_get_verify_result(ssl));
    if (hret != X509_V_OK)
    {
