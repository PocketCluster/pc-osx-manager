...
class DHCPOffer:
    ...
    def unpack(self):
        if self.data[4:8] == self.transID :
            self.offerIP = '.'.join(map(lambda x:str(int(x.encode('hex'),16)), data[16:20]))
            self.nextServerIP = '.'.join(map(lambda x:str(int(x.encode('hex'),16)), data[20:24]))  #c'est une option
            self.DHCPServerIdentifier = '.'.join(map(lambda x: str(int(x.encode('hex'),16)), data[245:249]))
            self.leaseTime = str(struct.unpack('!L', data[251:255])[0])
            self.router = '.'.join(map(lambda x: str(int(x.encode('hex'),16)) , data[257:261]))
            self.subnetMask = '.'.join(map(lambda x: str(int(x.encode('hex'),16)), data[263:267]))

            #dnsNB = int(data[268]/4)
            dnsNB = int(int(data[268].encode('hex'), 16) / 4)
            for i in range(0, 4 * dnsNB, 4):
                self.DNS.append('.'.join(map(lambda x:str(int(x.encode('hex'),16)), data[269 + i :269 + i + 4])))
