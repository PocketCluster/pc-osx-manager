...
+ (NSArray*) promptForPassword {
    ...
    NSDictionary *panelDict = [NSDictionary dictionaryWithObjectsAndKeys:
                               ...
                               @"PocketCluster Requested Administrator Privileges", kCFUserNotificationAlertMessageKey,
...
+ (NSArray *) promptForPassword:(NSString*)hostname user:(NSString*) username {
    ...
    [returnArray replaceObjectAtIndex:0 withObject:(__bridge_transfer NSString*)passwordRef];
...
+ (void) removePasswordForHost:(NSString*)hostname user:(NSString*)username {
    ...
    findKeychainItemStatus = [PasswordHelper
                              getPasswordKeychain:&passwordData
                              length:&passwordLen
                              itemRef:&itemRef
                              ...
                              user:(NSString*) usernameStr
{
    ...
    NSUInteger hostnameLength = [hostnameStr length];
    const char* hostname=[hostnameStr UTF8String];
    NSUInteger usernameLength = [usernameStr length];
...
+ (OSStatus) changePasswordKeychain:(SecKeychainItemRef)itemRef password:(NSString*) newPassword 
{
    ...
    NSUInteger passwordLength = strlen(cnewpassword);
