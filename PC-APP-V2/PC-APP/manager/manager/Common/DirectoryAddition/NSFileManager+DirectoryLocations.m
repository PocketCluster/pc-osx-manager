...
- (NSString *)applicationSupportDirectory
{
	...
	if (!result)
	{
		Log(@"Unable to find or create application support directory:\n%@", error);
	}
	return result;
}
...
- (void)recursiveRemoveFileOrFolder:(NSString *)fileFullPathName removeItself:(BOOL)removeItself;
{
	//Log(@"To be removed folder/file: %@", fileFullPathName);
	
	BOOL isDir = FALSE;
	
	if ([self fileExistsAtPath:fileFullPathName isDirectory:&isDir])
	{
		NSError * __autoreleasing err = nil;
		if (isDir)
		{
			NSArray *files = [self contentsOfDirectoryAtPath:fileFullPathName error:&err];
			...
			else
			{
				...
				{
					[self recursiveRemoveFileOrFolder:[fileFullPathName stringByAppendingPathComponent: file] removeItself:TRUE];
				}
			}
		}
		...
		if (removeItself)
		{
			if (![self removeItemAtPath:fileFullPathName error:&err])
			{
				Log(@"error %@", [err localizedDescription]);
			}
		}
