...
func Test_KeyExchange_CryptoCheck_TimeoutFail(t *testing.T) {
    ...
    // --- test
    slaveTS = masterTS.Add(time.Second)
    sa, end, err = slagent.TestSlaveCheckCryptoStatus(masterAgentName, "INCORRECT-SLAVE-NAME", pcrypto.TestSlaveSSHKey(), mb.(*masterBeacon).state.(DebugState).AESCryptor(), slaveTS)
    if err != nil {
        t.Error(err.Error())
        return
    }
