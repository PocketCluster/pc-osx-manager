...
func InitPackageKillRoutePath(appLife route.Router, feeder route.ResponseFeeder) {

    // install a package
    appLife.POST(routepath.RpathPackageKill(), func(_, rpath, payload string) error {
        ...
        if err != nil {
            return feedError(feeder, rpath, packageFeedbackKill, errors.WithMessage(err, "unable to specify package id"))
        }

        // 2. load template
        cTempl, err := loadComposeTemplate(pkgID)
        if err != nil {
            return feedError(feeder, rpath, packageFeedbackKill, errors.WithMessage(err, "unable to access package template"))
        }

        // 3. build client
        opts, err := newComposeClient()
        if err != nil {
            return feedError(feeder, rpath, packageFeedbackKill, errors.WithMessage(err, "unable to build orchestration client"))
        }
        ...
        if err != nil {
            return feedError(feeder, rpath, packageFeedbackKill, errors.WithMessage(err, "unable to create project"))
        }
