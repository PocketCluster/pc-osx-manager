...
DOCKER_OPTS="-H tcp://0.0.0.0:2375 --dns 127.0.0.1 --tlsverify --tlscacert=/etc/pocket/pki/pc_cert_auth.crt --tlscert=/etc/pocket/pki/pc_core_engine.crt --tlskey=/etc/pocket/pki/pc_core_engine.pem --cluster-advertise=eth0:2375 --cluster-store=etcd://pc-master:2379 --cluster-store-opt kv.cacertfile=/etc/pocket/pki/pc_cert_auth.crt --cluster-store-opt kv.certfile=/etc/pocket/pki/pc_core_engine.crt --cluster-store-opt kv.keyfile=/etc/pocket/pki/pc_core_engine.pem"`)
}
...
func SetupDockerAuthorityCert(rootPath string) error {
    var (
        slaveAuthCertFile string  = path.Join(rootPath, CoreAuthCertFileName)
