...
class tCacheOperation : public IFileHandler, public tSpecOpDetachable
{
    ...
    protected:
        enum enumIndexType : uint_least8_t
        {
            EIDX_UNSUPPORTED = 0
            ,EIDX_RELEASE
            ,EIDX_PACKAGES
        ...
        {
            bool vfile_ondisk = false
                ,uptodate = false
                ,parseignore = false
                ,hideDlErrors = false
                ,forgiveDlErrors = false
                ,alreadyparsed = false;
        ...
        bool Download(const std::string& sFilePathRel
                      ,bool bIndexFile
                      ,eDlMsgPrio msgLevel
                      ,tGetItemDelegate *p = NULL
                      ,const char *pForcedURL = NULL);
        ...
        void TellCount(unsigned int nCount, off_t nSize);
        ...
        bool Inject(cmstring &fromRel
                    ,cmstring& toRel
                    ,bool bSetIfileFlags = true
                    ,const header* pForcedHeader = NULL
                    ,bool bTryLink = false);
    ...
    public:
        ...
        {
            tStrDeq paths;
            tContId diffIdxId, bz2VersContId;
        };
        typedef std::map<tContId, tClassDesc> tContId2eqClass;
        
    private:
        
        bool Propagate(cmstring &donorRel
                       ,tContId2eqClass::iterator eqClassIter
        ...
        bool PatchFile(cmstring &srcRel
                       ,cmstring &patchIdxLocation
                       ,tPListConstIt pit
                       ,tPListConstIt itEnd
