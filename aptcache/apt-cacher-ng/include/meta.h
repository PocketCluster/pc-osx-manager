...
#define startsWith(where, what) \
    (0==(where).compare(0, (what).size(), (what)))
...
#define stripSuffix(where, what) \
    if(endsWithSzAr(where, what)) \
        where.erase(where.size()-sizeof(what)+1);

#define stripPrefixChars(where, what) \
    where.erase(0, where.find_first_not_of(what))

#define setIfNotEmpty(where, cand) \
{ \
    if(where.empty() && !cand.empty()) \
        where = cand; \
}

#define setIfNotEmpty2(where, cand, alt) \
{ \
    if(where.empty()) \
    { \
        if(!cand.empty()) \
...
class tHttpUrl
{
    public:
        ...
        {
            return a.sHost==sHost
                && a.sPort == sPort
                && a.sPath == sPath
                && a.sUserPass == sUserPass
#ifdef HAVE_SSL
                && a.bSSL == bSSL
#endif
        ...
        inline cmstring& GetPort() const
        {
            return !sPort.empty() ? sPort : GetDefaultPortForProto();
        }
...
#define POKE(x) for(;;) \
    { \
        ssize_t n = write(x, "", 1); \
        if (n>0 || (EAGAIN!=errno && EINTR!=errno)) \
            break; \
    }
...
#define pathTidy(s) \
{ \
    if(startsWithSz(s, "." SZPATHSEP)) s.erase(0, 2); tStrPos n(0); \
...
static inline void ignore_value (ssize_t i)
{
    (void) i;
}

static inline void ignore_ptr (void* p)
{
    (void) p;
}
...
struct extended_bool
{
    ...
    inline extended_bool(bool val, Textra xtra = defval):
        value(val)
        ,xdata(xtra)
    {
    };
