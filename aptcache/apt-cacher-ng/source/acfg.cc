...
namespace acfg {
    ...
    void _AddHooksFile(cmstring& vname)
    {
        ...
        while (itor.Next())
        {
            ...
            {
                errno = 0;
                unsigned long n = strtoul(val.c_str(), NULL, 10);
                if (!errno)
                    hs.downDuration = n;
            }
        }
    ...
    bool GetRepNameAndPathResidual(const tHttpUrl& in
                                   ,string& sRetPathResidual
    ...
    void ReadBackendsFile(const string & sFile, const string &sRepName)
    {
        ...
        while (itor.Next())
        {
            ...
            if (entry.SetHttpUrl(itor.sLine) || (itor.sLine.empty() && ! entry.sHost.empty() && ! entry.sPath.empty()))
            {
    ...
    void ReadRewriteFile(const string & sFile, cmstring& sRepName)
    {
        ...
        while (reader.GetOneLine(sLine))
        {
            ...
            if (sLine.empty()) // end of block, eof, ... -> commit it
            {
                ...
                {
                    BARF("Parse error, missing Site: field around line " << sFile << ":"<< reader.GetCurrentLine());
                }
                ...
                for (const auto& host : hosts)
                {
                    for (const auto& path : paths)
                    {
                        ...
                        url.sHost = host;
                        url.sPath = path;
                        mapUrl2pVname[url.sHost + ":" + url.GetPort()].push_back(make_pair(url.sPath, GetRepoEntryRef(sRepName)));
#ifdef DEBUG
                        cerr << "Mapping: "<< host << path << " -> "<< sRepName <<endl;
#endif
                    }
                }
                hosts.clear();
            ...
            {
                BARF("Error parsing rewrite definitions, around line " << sFile << ":"<< reader.GetCurrentLine() << " : " << sLine);
            }
    ...
    void PostProcConfig(bool bDumpConfig)
    {
        ...
        if (!rechecks::CompileUncExpressions(rechecks::NOCACHE_REQ, tmpDontcacheReq.empty() ? tmpDontcache : tmpDontcacheReq)
           || !rechecks::CompileUncExpressions(rechecks::NOCACHE_TGT, tmpDontcacheTgt.empty() ? tmpDontcache : tmpDontcacheTgt))
        {
        ...
        {
            usewrap = (qgrep("apt-cacher-ng", "/etc/hosts.deny") || qgrep("apt-cacher-ng", "/etc/hosts.allow"));
#ifndef HAVE_LIBWRAP
...
#warning adding hook control pins
        for(tMapString2Hostivec::iterator it = repoparms.begin(); it != repoparms.end() ; ++it)
        {
...
namespace rechecks
{
    ...
    struct
    {
        regex_t *pat = nullptr, *extra = nullptr;
    } rex[ematchtype_max];
...
void mkbasedir(const string & path)
{
    ...
    if (startsWith(path, acfg::cacheDirSlash))
    {
        ...
        pos = static_cast<unsigned int>(path.find("/", acfg::cachedir.size() + 1));
    }
    
    for(; pos<path.size(); pos = static_cast<unsigned int>(path.find(SZPATHSEP, pos + 1)))
    {
        if (pos > 0)
