...
class tBzDec : public IDecompressor
{
    ...
    public:
        ...
        virtual bool UncompMore(char *szInBuf, size_t nBufSize, size_t &nBufPos, acbuf &UncompBuf) override
        {
            ...
            strm.avail_in = static_cast<unsigned int>(nBufSize - nBufPos);
            strm.next_out = UncompBuf.wptr();
            strm.avail_out = static_cast<unsigned int>(UncompBuf.freecapa());
            
            int ret = BZ2_bzDecompress(&strm);
            if (ret == BZ_STREAM_END || ret == BZ_OK)
            {
                nBufPos = nBufSize - strm.avail_in;
                UINT nGotBytes = static_cast<unsigned int>(UncompBuf.freecapa() - strm.avail_out);
...
static const uint8_t bz2Magic[] = { 'B', 'Z', 'h' };
#endif
    ...
    static const uint8_t gzMagic[] = { 0x1f, 0x8b, 0x8 };
#endif
    ...
    class tXzDec : public IDecompressor
    {
        ...
        public:
            ...
            {
                auto x = (lzmaFormat
                          ? lzma_alone_decoder(&strm, EXTREME_MEMORY_SAVING ? 32000000 : MAX_VAL(uint64_t))
                          : lzma_stream_decoder(&strm, EXTREME_MEMORY_SAVING ? 32000000 : MAX_VAL(uint64_t), LZMA_TELL_UNSUPPORTED_CHECK | LZMA_CONCATENATED));
...
bool filereader::GetOneLine(string & sOut, bool bForceUncompress)
{
    ...
    if (m_Dec.get())
    {
        if ((!m_nBufPos || bForceUncompress) && !m_Dec->eof)
        {
            ...
            {
                if (m_UncompBuf.totalcapa() >= BUFSIZEMAX || !m_UncompBuf.setsize(m_UncompBuf.totalcapa() * 2))
                {
                    m_bError = true;
    ...
    class csumSHA1 : public csumBase, public SHA_INFO
    {
        public:
            ...
            {
                sha_update(this, (SHA_BYTE*) data, static_cast<int>(size));
            }
            void finish(uint8_t* ret) override
            {
                sha_final(ret, this);
            }
    };
    ...
    class csumMD5 : public csumBase, public md5_state_s
    {
        public:
            ...
            void add(const char *data, size_t size) override
            {
                md5_append(this, (md5_byte_t*) data, static_cast<int>(size));
            }
    ...
    auto_ptr<csumBase> csumBase::GetChecker(CSTYPES type)
    {
        switch(type)
        {
            case CSTYPE_MD5: return auto_ptr<csumBase>(new csumMD5);
            case CSTYPE_SHA1:
            default: return auto_ptr<csumBase>(new csumSHA1);
        }
    }

    bool filereader::GetChecksum(const mstring& sFileName
                                 ,int csType
                                 ,uint8_t out[]
                                 ,bool bTryUnpack
                                 ,off_t& scannedSize
    ...
    {
        filereader f;
        return (f.OpenFile(sFileName, !bTryUnpack) && f.GetChecksum(csType, out, scannedSize, fDump));
    }
    ...
    bool filereader::GetChecksum(int csType, uint8_t out[], off_t &scannedSize, FILE *fDump)
    {
        ...
        else
        {
            for(;;)
            {
                ...
                UINT nPlainSize = static_cast<unsigned int>(m_UncompBuf.size());
        ...
        if (NULL != (f = fopen(pathOut, "w")))
        {
            if (!ferror(f))
            {
                if (NULL != (bzf = BZ2_bzWriteOpen( &nError, f, 9, 0, 30)))
                {
                    if (BZ_OK == nError)
                    {
                        BZ2_bzWrite(&nError, bzf, (void*) reader.GetBuffer(), static_cast<int>(reader.GetSize()));
                        if (BZ_OK == nError)
                            bRet = true;
                    }
