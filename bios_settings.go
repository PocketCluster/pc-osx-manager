...
#include "wrapper/src/bios_settings.c"
*/
...
func (settings *BiosSettings) GetLogoFadeIn() (bool, error) {
    ...
    result := C.VboxGetBiosSettingsLogoFadeIn(settings.csettings, &clogoFadeIn)
    if C.VboxFAILED(result) != 0 {
        return false, vboxError("Failed to get IBiosSettings logoFadeIn: %x", result)
    }
...
func (settings *BiosSettings) SetLogoFadeIn(logoFadeIn bool) error {
    ...
    result := C.VboxSetBiosSettingsLogoFadeIn(settings.csettings, clogoFadeIn)
    if C.VboxFAILED(result) != 0 {
        return vboxError("Failed to set IBiosSettings logoFadeIn: %x", result)
    }
    return nil
}
...
func (settings *BiosSettings) GetLogoFadeOut() (bool, error) {
    ...
    result := C.VboxGetBiosSettingsLogoFadeOut(settings.csettings, &clogoFadeOut)
    if C.VboxFAILED(result) != 0 {
        return false, vboxError("Failed to get IBiosSettings logoFadeOut: %x", result)
    }
...
func (settings *BiosSettings) SetLogoFadeOut(logoFadeOut bool) error {
    ...
    result := C.VboxSetBiosSettingsLogoFadeOut(settings.csettings, clogoFadeOut)
    if C.VboxFAILED(result) != 0 {
        return vboxError("Failed to set IBiosSettings logoFadeOut: %x", result)
    }
    return nil
}
...
func (settings *BiosSettings) GetBootMenuMode() (BootMenuMode, error) {
    ...
    result := C.VboxGetBiosSettingsBootMenuMode(settings.csettings, &cmenuMode)
    if C.VboxFAILED(result) != 0 {
        return 0, vboxError("Failed to get IBiosSettings boot menu mode: %x", result)
    }
...
func (settings *BiosSettings) SetBootMenuMode(menuMode BootMenuMode) error {
    result := C.VboxSetBiosSettingsBootMenuMode(settings.csettings, C.PRUint32(menuMode))
    if C.VboxFAILED(result) != 0 {
        return vboxError("Failed to set IBiosSettings boot menu mode: %x", result)
    }
    return nil
}
...
func (settings *BiosSettings) Release() error {
    if settings.csettings != nil {
        result := C.VboxIBiosSettingsRelease(settings.csettings)
        if C.VboxFAILED(result) != 0 {
            return vboxError("Failed to release IBiosSettings: %x", result)
        }
...
func (machine *Machine) GetBiosSettings() (BiosSettings, error) {
    ...
    result := C.VboxGetMachineBIOSSettings(machine.cmachine, &settings.csettings)
    if C.VboxFAILED(result) != 0 {
        return settings, vboxError("Failed to get IMachine BIOS settings: %x", result)
    }
    return settings, nil
}
